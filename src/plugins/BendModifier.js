...
var BendModifier = function () {
    ...
    this.modify = function (geometry) {
        ...
        var meshDepth =
            meshGeometryBoundingBoxMaxy - meshGeometryBoundingBoxMiny;
        var ParamB =
            (2 * this._sinhInverse(Math.tan(this.angle))) / meshWidthold;
        var oldMiddlex =
        ...
        for (var i = 0; i < vertices.length; i++) {
            ...
            newVertices[i].x =
                ((this._sign(newVertices[i].x - oldMiddlex) * 1) / ParamB) *
                this._sinhInverse((newVertices[i].x - oldMiddlex) * ParamB);
        }

        var meshWidth =
            (2 / ParamB) * this._sinhInverse((meshWidthold / 2) * ParamB);

        var NewParamB =
            (2 * this._sinhInverse(Math.tan(this.angle))) / meshWidth;
        ...
        for (var i = 0; i < vertices.length; i++) {
            var x0 =
                ((this._sign(oldVertices[i].x - oldMiddlex) * 1) / ParamB) *
                this._sinhInverse((oldVertices[i].x - oldMiddlex) * ParamB);
            var y0 =
                (1 / NewParamB) * this._cosh(NewParamB * x0) - 1 / NewParamB;
        ...
        const posAttrib = new BufferAttribute(
            new Float32Array(new_buf_vertices),
            3
        );
